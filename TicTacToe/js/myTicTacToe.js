...
function rollForTurn() {
    ...
    if (pOne > pTwo) {
        first = "PLayer 1";
        setTimeout(function(){ txt1 = txt1 + "PLayer 1 winds, please choose a square.";}, 2000);
...
function saveSettings() {
    ...
    if (p1Selected[p1Index].text == p1Selected[p1Index].text) {
...
function determineAvatar() {
    ...
    var active = document.getElementById('showPlayers').innerHTML; // get active player
...
function checkWinCon1(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
            var match0Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "1") {
            var match1Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "2") {
            var match2Avatar = info[i]; // only interested in recording the avatar
        }
    }
...
function checkWinCon1(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "3") {
            var match3Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "4") {
            var match4Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "5") {
            var match5Avatar = info[i]; // only interested in recording the avatar
        }
    }
    // this will trigger (ONLY) if there was a match for index0, index1, and index2
...
function checkWinCon1(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "6") {
            var match6Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "7") {
            var match7Avatar = info[i]; // only interested in recording the avatar
        }
        if (info[i].charAt(0) == "8") {
            var match8Avatar = info[i]; // only interested in recording the avatar
        }
    }
    // this will trigger (ONLY) if there was a match for index0, index1, and index2
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { //if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty then the square is unoccupied.
            var paintAvatar = determineAvatar(); // get the correct avatar to paint for the active
